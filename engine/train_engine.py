...
class TrainEngine(object):
    ...
    def run(self, save_dir='log', max_epoch=0, start_epoch=0, fixbase_epoch=0, open_layers=None,
            ...
            ranks=[1, 5, 10, 20], rerank=False, visactmap=False):
    ...
    def test(self, epoch, testloader, dist_metric='euclidean', normalize_feature=False,
             visrank=False, visrank_topk=10, save_dir='', ranks=[1, 5, 10, 20], rerank=False):
    ...
    def _evaluate(self, epoch, dataset_name='', queryloader=None, galleryloader=None,
                  ...
                  visrank_topk=10, save_dir='', ranks=[1, 5, 10, 20], rerank=False):
        torch.cuda.empty_cache()
        ...
        for batch_idx, data in enumerate(queryloader):
            ...
            #
            # b, c, h, w = imgs.size()
            # imgs_crop_rb = resize(crop_rb(imgs, h - 10, w - 10), (h, w), interpolation='bilinear')
            # imgs_crop_lb = resize(crop_lb(imgs, h - 10, w - 10), (h, w), interpolation='bilinear')
            # imgs_crop_lt = resize(crop_lt(imgs, h - 10, w - 10), (h, w), interpolation='bilinear')
            # imgs_crop_rt = resize(crop_rt(imgs, h - 10, w - 10), (h, w), interpolation='bilinear')
            # imgs_crop_center = resize(crop_rt(imgs, h - 10, w - 10), (h, w), interpolation='bilinear')
            #
            # features0 = self._extract_features(imgs)
            # features1 = self._extract_features(imgs_hflip)
            # features2 = self._extract_features(imgs_crop_rb)
            # features3 = self._extract_features(imgs_crop_lb)
            # features4 = self._extract_features(imgs_crop_lt)
            # features5 = self._extract_features(imgs_crop_rt)
            # features6 = self._extract_features(imgs_crop_center)
            #
            # features = (features0 + features1 + features2 + features3 + features4 + features5 + features6) / 7.
