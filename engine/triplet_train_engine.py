...
class ImageTripletEngine(TrainEngine):
    ...
    def __init__(self, datamanager, model, optimizer, margin=0.3,
                 ...
                 metric='cosine', label_smooth=True, ranked_loss=False, ms_loss=False, apex=False):
        super(ImageTripletEngine, self).__init__(datamanager, model, optimizer, scheduler, use_gpu, apex)
        ...
        if ranked_loss:
            # self.criterion_t = RankedLoss(margin=1.2, alpha=2., tval=1.0)
            # self.metric = 'euclidean'
            # self.criterion_t = CRankedLoss(margin=1.3, alpha=2., tval=1.0)
            self.criterion_t = RankedLoss(margin=1.2, alpha=2., tval=1.0, metric=metric)
        elif ms_loss:
            self.criterion_t = MultiSimilarityLoss(thresh=0.5, margin=0.5, scale_pos=2., scale_neg=40.)
        else:
    ...
    def train(self, epoch, max_epoch, trainloader, fixbase_epoch=0, open_layers=None, print_freq=10, **kwargs):
        ...
        for batch_idx, data in enumerate(trainloader):
            ...
            loss_t = self._compute_loss(self.criterion_t, features, pids, kwargs['loss_reduce']) * self.weight_t
            loss_x = self._compute_loss(self.criterion_x, outputs, pids, kwargs['loss_reduce']) * self.weight_x
            loss = loss_t + loss_x
