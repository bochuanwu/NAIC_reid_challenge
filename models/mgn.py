...
_all__ = ['mgn_seresnet101', 'mgn_resnet50', 'mgn_resnet101']
...
class MGN(nn.Module):
    def __init__(self, resnet, num_classes, local_dim=512):
        ...
        res_conv5_global_bn = nn.BatchNorm1d(2048)
        ...
        res_conv5_global_fc = nn.Linear(2048, num_classes, bias=False)
    ...
    def forward(self, x):
        ...
        global_softmax_feature_p1 = global_feature_p1.flatten(1)
        global_softmax_feature_p2 = global_feature_p2.flatten(1)
        global_softmax_feature_p3 = global_feature_p3.flatten(1)
        ...
        global_softmax_feature_p1 = self.res_conv5_bn_0(global_softmax_feature_p1)
        global_softmax_feature_p2 = self.res_conv5_bn_1(global_softmax_feature_p2)
        global_softmax_feature_p3 = self.res_conv5_bn_2(global_softmax_feature_p3)
...
def mgn_resnet50_256(num_classes, loss='softmax', **kwargs):
    resnet = resnet50_ibn_a(num_classes, loss=loss, **kwargs)
    model = MGN(resnet, num_classes, local_dim=256)
    return model


def mgn_resnet50_512(num_classes, loss='softmax', **kwargs):
    resnet = resnet50_ibn_a(num_classes, loss=loss, **kwargs)
    model = MGN(resnet, num_classes, local_dim=512)
    return model
