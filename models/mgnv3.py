...
from models import resnet50_ibn_a, resnet101_ibn_a
from .kaiming_init import weights_init_classifier, weights_init_kaiming
from .seresnet_ibn_a import seresnet101_ibn_a

_all__ = ['mgnv3_resnet50_256', 'mgnv3_resnet50_512', 'mgnv3_resnet101_256', 'mgnv3_resnet101_512']
...
class MGNv3(nn.Module):
    ...
    def forward(self, x):
        ...
        return y, global_softmax_feats


def mgnv3_resnet50_256(num_classes, loss='softmax', **kwargs):
    resnet = resnet50_ibn_a(num_classes, loss=loss, **kwargs)
    model = MGNv3(resnet, num_classes, local_dim=256)
    return model


def mgnv3_resnet50_512(num_classes, loss='softmax', **kwargs):
    resnet = resnet50_ibn_a(num_classes, loss=loss, **kwargs)
    model = MGNv3(resnet, num_classes, local_dim=512)
    return model

def mgnv3_resnet101_256(num_classes, loss='softmax', **kwargs):
    resnet = resnet101_ibn_a(num_classes, loss=loss, **kwargs)
    model = MGNv3(resnet, num_classes, local_dim=256)
    return model


def mgnv3_resnet101_512(num_classes, loss='softmax', **kwargs):
    resnet = resnet101_ibn_a(num_classes, loss=loss, **kwargs)
    model = MGNv3(resnet, num_classes, local_dim=512)
    return model
