...
AVAI_SCH = ['single_step', 'multi_step', 'cosine', 'warmup_linear', 'cosine_warm_restarts']


def build_lr_scheduler(optimizer, lr_scheduler='single_step',
                       stepsize=1, gamma=0.1, max_epoch=1,
                       warmup_factor=1.0 / 3, warmup_iters=1,
                       t_mult=2, eta_min=3e-6
                       ):
    ...
    Args:
        ...
        stepsize (int or list, optional): step size to decay learning rate. When ``lr_scheduler``
            ...
            "multi_step", "warmup_linear" or "warmup_constant", ``stepsize`` is a list. Default is 1.
        gamma (float, optional): decay rate. Default is 0.1.
        max_epoch (int, optional): maximum epoch (for cosine annealing). Default is 1.
        warmup_factor (float, optional):  warmup rate to scale learning rate. Only when ``lr_scheduler``
            is "warmup_linear".
        warmup_iters (int, optional):  warmup size to maximum learning rate. Only when ``lr_scheduler``
            is "warmup_linear".
    Examples::
    ...
    if lr_scheduler == 'single_step':
        ...
        scheduler = WarmupMultiStepLR(optimizer, milestones=stepsize, gamma=gamma, warmup_factor=warmup_factor,
