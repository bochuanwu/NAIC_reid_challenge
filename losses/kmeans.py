...
def cluster(X, n_centers, n_iters=100):
    centers = random_init(X, n_centers)
    codes = compute_codes(X, centers)
    num_iterations = 0
    for i in range(n_iters):
        num_iterations += 1
        centers = update_centers(X, codes, n_centers)
        new_codes = compute_codes(X, centers)
