#!/usr/bin/env python
# -*- coding:utf-8 -*-
# @Time    : 2019/11/13 10:20 上午
# @author  : wuh-xmu
# @FileName: ranked_clu_loss.py
# @Software: PyCharm
...
from losses.kmeans import cluster
...
class CRankedLoss(object):
    ...
    def __call__(self, global_feat, labels, normalize_feature=True):
        ...
        centroid, appearanceGroups = cluster(X=clu_features, n_centers=8, n_iters=10)
        allGroups = torch.unique(appearanceGroups)

        total_loss = 0
        for group in allGroups:
            indices = torch.where(appearanceGroups == group)
